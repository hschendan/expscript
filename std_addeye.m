...
global nraw data_path subject ch64 CAR RTlock

disp('Add eye channels ...')

for s=1:nraw
    sname = [data_path{s} subject{s} '_cebM12e.set'];        
    if exist(sname, 'file')>0
        ...
        EEG = pop_loadset('filename', [subject{s} '_cebM12e.set'], 'filepath', data_path{s});                
        ...
        if CAR
            sname = [data_path{s} subject{s} '_cebCARe.set'];        
            if exist(sname, 'file')>0
                clear EEG;  % Clear is to make sure that we start fresh for each subject with the EEG structures
                EEG = pop_loadset('filename', [subject{s} '_cebCARe.set'], 'filepath', data_path{s}); 
                if ch64 % 64 channel cap            
                    EEG = pop_eegchanoperator( EEG, { 'ch70 = ch65 - ch66 label HE' 'ch71 = ch34 - ch67 label VE' 'ch72 = ch34 label Fp2c' 'ch73 = ch67 label LEc' });
                else    % 32 channel cap
                    EEG = pop_eegchanoperator( EEG, { 'ch38 = ch33 - ch34 label HE' 'ch39 = ch30 - ch35 label VE' 'ch40 = ch30 label Fp2c' 'ch41 = ch35 label LEc'});
    ...
    % RT lock ERP
    if RTlock
        sname = [data_path{s} subject{s} '_RT_cebM12e.set'];        
        if exist(sname, 'file')>0
            disp('Add eye channels for RT-locked: WORKING ...')
            clear EEG;  % Clear is to make sure that we start fresh for each subject with the EEG structures
            EEG = pop_loadset('filename', [subject{s} '_RT_cebM12e.set'], 'filepath', data_path{s});                
            if ch64 % 64 channel cap
