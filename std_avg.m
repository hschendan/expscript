...
if artif_one
    ...
    if exist(sname, 'file')>0               
        ...
        erpname = [subject{n} '_ar'];  % name for erpset menu
        ...
        values = pop_rt2text(ERP, 'arfilter', 'on', 'eventlist',  1, 'filename', [data_path{n} erpname '_erp_RT.txt'], 'header',' on', 'listformat', 'basic' );
        values = pop_rt2text(ERP, 'arfilter', 'on', 'eventlist',  1, 'filename', [data_path{n} erpname '_erp_RTs.txt'], 'header', 'on', 'listformat', 'itemized' );
        % All Trials
        ERP = pop_averager( EEG , 'Criterion', 'all', 'ExcludeBoundary', 'on', 'SEM', 'on' );
        erpname = [subject{n} '_al'];  % name for erpset menu
        fname_erp= fullfile(data_path{n}, [erpname '.erp']);
        pop_savemyerp(ERP, 'erpname', erpname, 'filename', fname_erp, 'Warning', 'off'); 
        % Artifact Trials
        ERP = pop_averager( EEG , 'Criterion', 'bad', 'ExcludeBoundary', 'on', 'SEM', 'on' );
        erpname = [subject{n} '_ba'];  % name for erpset menu
        fname_erp= fullfile(data_path{n}, [erpname '.erp']);
        pop_savemyerp(ERP, 'erpname', erpname, 'filename', fname_erp, 'Warning', 'off'); 
        % Get Evoked spectra, Good trials, no artifacts
        ERP = pop_averager( EEG , 'Compute', 'EFFT', 'Criterion', 'good', 'ExcludeBoundary', 'on', 'SEM', 'on' );
        erpname = [subject{n} '_ae'];  % name for erpset menu
        fname_erp= fullfile(data_path{n}, [erpname '.erp']);
        pop_savemyerp(ERP, 'erpname', erpname, 'filename', fname_erp, 'Warning', 'off');  
        % RTs
    else
