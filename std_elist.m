...
for s=1:nraw
    ...
    sname = [data_path{s} subject{s} '_c.set'];       
    if exist(sname, 'file')>0
        
        % Load dataset
        fprintf('\n++++\n+ Loading dataset for %s ... \n++++\n\n', subject{s} );
        EEG = pop_loadset('filename', [subject{s} '_c.set'], 'filepath', data_path{s});                
        
        % Creates basic EVENTLIST (Option 1)
        fprintf('\n++++\n+ Creating basic EVENTLIST (Option 1) for %s ... \n++++\n\n', subject{s} );
        EEG = pop_creabasiceventlist(EEG, 'AlphanumericCleaning', 'on', 'BoundaryNumeric', { -99 }, 'BoundaryString', { 'boundary' }, 'Eventlist', [data_path{s} subject{s} '_e_list.txt'], 'Warning', 'on' );               
